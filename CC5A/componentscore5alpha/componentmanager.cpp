...
bool ComponentManager::isInitDone ()
{
    if (_instance == nullptr)
        return false;
...
void ComponentManagerPrivate::shutdown ()
{
    ...
    for (ComponentSpec *spec : qAsConst (_initializedQueue))
    {
        ...
        if (IComponent::AsynchronousShutdown == flag)
        {
            ...
            //QString messageString = QString ("[ComponentsCore] Component '%1' stopped 'AsynchronousShutdown'")
            //        .arg (spec->name ());
            //qInfo () << qPrintable (messageString);
            qInfo () << qPrintable (QString ("[ComponentsCore] Component '%1' stopped 'AsynchronousShutdown'")
                                    .arg (spec->name ()));
        }
        else
        {
            //QString messageString = QString ("[ComponentsCore] Component '%1' stopped 'SynchronousShutdown'")
            //        .arg (spec->name ());
            //qInfo () << qPrintable (messageString);
    ...
    for (ComponentSpec *spec : qAsConst (_initializedQueue))
    {
        ...
        if (uflag == IComponent::NonBlockingUnload) {
            ...
            //QString messageString = QString ("[ComponentsCore] Component '%1' unload with wait ...")
            //        .arg (spec->name ());
            //qInfo () << qPrintable (messageString);
...
void ComponentManagerPrivate::loadComponents ()
{
    ...
    for (auto spec : queue)
    {
        ...
        if (bload)
        {
            ...
            //        = QString ("[ComponentsCore] Component loaded successfully '%1' (version: '%2')")
        ...
        {
            //QString errorString
            //        = QString ("[ComponentsCore] Component load failed '%1' (version: '%2')")
            //        .arg (spec->name (), spec->version ());
            //qInfo () << qPrintable (errorString);
            qInfo () << qPrintable (QString ("[ComponentsCore] Component load failed '%1' (version: '%2')")
                                    .arg (spec->name (), spec->version ()));
    ...
    for (ComponentSpec *spec : loadedQueue)
    {
        ...
        if (binit)
        {
            ...
            QString messageString
                    = QString ("[ComponentsCore] Component initialized successfully '%1' (version: '%2')")
                    .arg (spec->name (), spec->version ());
            qInfo () << qUtf8Printable (messageString);
            emit q->signalComponentInitialized (spec->name (), spec->version (), true);
        }
        else
        {
            //QString errorString
            //        = QString ("[ComponentsCore] Component '%1' (version: '%2') initialization failed")
            //        .arg (spec->name (), spec->version ());
            //qCritical () << qUtf8Printable (errorString);
